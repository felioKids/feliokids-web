  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f4f0' }}>
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
            {CATS.map(cat => (
              <button key={cat.id}
                ...
                style={{
                  borderRadius: 15, overflow: 'hidden', aspectRatio: '1', position: 'relative',
                  border: 'none', transition: 'all .2s',
                  boxShadow: activeCat === cat.id ? `0 0 0 3px ${cat.c}, 0 6px 20px rgba(0,0,0,0.15)` : '0 3px 12px rgba(0,0,0,0.09)',
              ...
              >
                ...
                <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 8, background: `linear-gradient(to top, ${cat.c}e0 0%, transparent 55%)` }}>
        ...
        {/* ── SUBCATEGORIES ── */}
        {activeCat && (
          <div className="fade-up" style={{ margin: '10px 14px 0', background: '#fff', borderRadius: 16, padding: '13px 14px', boxShadow: '0 3px 14px rgba(0,0,0,0.07)' }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#bbb', textTransform: 'uppercase', letterSpacing: '.6px', marginBottom: 9 }}>
              {CATS.find(c => c.id === activeCat)?.l} — choisir une sous-catégorie
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {CATS.find(c => c.id === activeCat)?.subs.map(s => (
                <button key={s}
                  onClick={() => clickSub(s)}
                  style={{ padding: '7px 12px', borderRadius: 9, fontSize: 12, fontWeight: 700, border: 'none', background: activeSub === s ? 'linear-gradient(135deg,#FF6B6B,#FF8E53)' : '#f0f0f0', color: activeSub === s ? '#fff' : '#555', transition: 'all .15s' }}>
                  {s}
                </button>
              ))}
            </div>
