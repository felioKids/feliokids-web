...
async function ensurePlaceExists(placeId, name) {
  const { data } = await supabase
    ...
    .single();
  if (!data) {
    await supabase.from('places').insert({ place_id: placeId, name });
  }
}
...
function aggregateQuickAnswers(quickAnswers) {
  ...
  return {
    parking: total > 0 ? Math.round((parkingOui / total) * 100) : null,
    poussette: total > 0 ? Math.round((poussetteOui / total) * 100) : null,
    topAge,
    topTime,
    total,
  };
...
function ReviewModal({ activity, onClose, onSuccess }) {
  const [step, setStep] = useState(1); // 1 = quick answers, 2 = text review
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (hasQuick) {
        await supabase.from('quick_answers').insert({
  ...
  return (
    ...
    >
      <div style={{
        background: '#FFF8F1', borderRadius: '20px 20px 0 0',
        ...
        {submitted ? (
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <h3 style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: '18px', color: '#1a1a1a', margin: '0 0 8px' }}>
              Merci pour ton avis !
            </h3>
            <p style={{ fontFamily: 'Outfit, sans-serif', fontSize: '13px', color: '#888', margin: 0 }}>
              Ton conseil sera visible après modération. Tu aides les familles ! 🙏
            </p>
          </div>
        ) : (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: '17px', color: '#1a1a1a', margin: 0 }}>
                Ton avis sur {activity.name}
              </h3>
              <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#aaa', padding: '0 4px' }}>✕</button>
            </div>

            {step === 1 && (
              <>
                <p style={{ fontFamily: 'Outfit, sans-serif', fontSize: '13px', color: '#888', margin: '0 0 16px' }}>
                  Réponds en 10 secondes — aide les autres familles ! 👇
                </p>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button style={quick.parking === false ? { ...btnActive, borderColor: '#ef4444', color: '#ef4444', background: '#fef2f2' } : btnBase} onClick={() => handleQuickToggle('parking', false)}>❌ Non</button>
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button style={quick.poussette === false ? { ...btnActive, borderColor: '#ef4444', color: '#ef4444', background: '#fef2f2' } : btnBase} onClick={() => handleQuickToggle('poussette', false)}>❌ Non</button>
                  </div>
                </div>
                ...
                <button
                  onClick={() => setStep(2)}
                  style={{ width: '100%', padding: '13px', background: '#FF6B4A', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '14px', fontFamily: 'Outfit, sans-serif', fontWeight: 700, cursor: 'pointer' }}
                >
                  Continuer → Laisser un avis écrit
                </button>
                <button
                  onClick={handleSubmit}
                  disabled={submitting}
                  style={{ width: '100%', padding: '10px', background: 'none', color: '#aaa', border: 'none', fontSize: '13px', fontFamily: 'Outfit, sans-serif', cursor: 'pointer', marginTop: '8px' }}
                >
            ...
            {step === 2 && (
              <>
                ...
                <div style={{ marginBottom: '14px' }}>
                  ...
                  <input
                    value={form.age_of_children}
                    onChange={e => setForm(p => ({ ...p, age_of_children: e.target.value }))}
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    {[1, 2, 3, 4, 5].map(s => (
                      <button key={s} onClick={() => setForm(p => ({ ...p, rating: s }))}
                        style={{ fontSize: '22px', background: 'none', border: 'none', cursor: 'pointer', opacity: form.rating >= s ? 1 : 0.3, transition: 'opacity 0.15s' }}>
                        ⭐
                      </button>
                    ))}
                  </div>
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <textarea
                    ...
                    placeholder="Ce qui t'a plu, ce qui pourrait être amélioré, conseils pratiques..."
                    rows={4}
                    style={{ width: '100%', padding: '10px 12px', borderRadius: '10px', border: '1.5px solid #e8e0d8', fontFamily: 'Outfit, sans-serif', fontSize: '14px', background: '#fff', boxSizing: 'border-box', resize: 'vertical', outline: 'none' }}
                  />
                </div>

                <div style={{ display: 'flex', gap: '10px' }}>
                  <button onClick={() => setStep(1)} style={{ flex: 1, padding: '12px', background: '#f5f0ea', color: '#555', border: '1.5px solid #e8e0d8', borderRadius: '12px', fontSize: '14px', fontFamily: 'Outfit, sans-serif', fontWeight: 600, cursor: 'pointer' }}>
                    ← Retour
                  </button>
                  <button
                    onClick={handleSubmit}
...
function ConseilsFamilles({ activity, onOpenModal }) {
  ...
  useEffect(() => {
    ...
    fetchFamilyData(activity.place_id).then(d => {
      setData(d);
      setLoading(false);
    });
  }, [activity.place_id]);
  ...
  return (
    <div
      onClick={e => e.stopPropagation()}
      style={{ borderTop: '1px solid #f0e8e0', marginTop: '12px', paddingTop: '14px' }}
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        <span style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: '13px', fontWeight: 700, color: '#1a1a1a' }}>
          👨‍👩‍👧 Conseils des familles
        </span>
        {reviews.length > 0 && (
          <span style={{ fontSize: '11px', color: '#aaa', fontFamily: 'Outfit, sans-serif' }}>
            {reviews.length} avis
          </span>
        )}
      </div>
        ...
        <>
          ...
          {agg && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '10px' }}>
              ...
              {agg.topAge && (
                <span style={chipStyle()}>👶 {agg.topAge} ans</span>
              )}
              {agg.topTime && (
                <span style={chipStyle('#7c3aed', '#f5f3ff')}>⏱️ {agg.topTime}</span>
              )}
            </div>
          )}
          ...
          {reviews.length === 0 ? (
            <button
              onClick={() => onOpenModal()}
              style={{ width: '100%', padding: '11px', background: 'linear-gradient(135deg, #FF6B4A, #ff8c6b)', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '13px', fontFamily: 'Outfit, sans-serif', fontWeight: 700, cursor: 'pointer', textAlign: 'center' }}
            >
            ...
            <>
              {(expanded ? reviews : reviews.slice(0, 1)).map(review => (
                <div key={review.id} style={{ background: '#fff', borderRadius: '10px', padding: '10px 12px', marginBottom: '8px', border: '1px solid #f0e8e0' }}>
                  ...
                  {review.review_text && (
                    <p style={{ margin: 0, fontSize: '12px', color: '#555', fontFamily: 'Outfit, sans-serif', lineHeight: 1.5 }}>
                      {review.review_text}
                    </p>
                  )}
                </div>
...
export default function ActivityCard({ activity, onSelect, distanceKm }) {
  ...
  const [reviewKey, setReviewKey] = useState(0); // force reload after submit
  ...
  const toggleFav = useCallback((e) => {
    ...
    try {
      ...
      const next = exists
        ...
        : [...favs, {
            place_id: activity.place_id, name: activity.name,
            address: activity.address, rating: activity.rating,
            photoReference: activity.photoReference, fallbackPhoto: activity.fallbackPhoto,
            catId: activity.catId, geometry: activity.geometry, types: activity.types,
          }];
